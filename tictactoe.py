...
class Board:
    ...
    def __eq__(self,other):
        for i in range(len(self.items)):
            for j in range(len(self.items)):
                if type(self.items[i][j]) != type(other.items[i][j]):
                    return False
        return True
    ...
    def eval(self):
        for i in range(len(self.items)):
            if self.items[i][0].eval() == self.items[i][1].eval() and self.items[i][2].eval() == self.items[i][1].eval():
                if self.items[i][0].eval != 0:
                    return self.items[i][0].eval()

            if self.items[0][i].eval() == self.items[1][i].eval() and self.items[2][i].eval() == self.items[1][i].eval():
                if self.items[0][i].eval() != 0:
                    return self.items[0][i].eval()
        
        if self.items[0][0].eval() == self.items[1][1].eval() and self.items[2][2].eval() == self.items[1][1].eval():
            if self.items[0][0] != 0:
                return self.items[0][0].eval()
            
        if self.items[0][2].eval() == self.items[1][1].eval() and self.items[1][1].eval() == self.items[2][0].eval():
            if self.items[0][2].eval() != 0:
                return self.items[0][2].eval()
    ...
    def full(self):
        for i in range(len(self.items)):
            for j in range(len(self.items)):
                if not isinstance(self.items[i][j], X) and not isinstance(self.items[i][j], O):
...
def minimax(player,board):
    ...
    for i in range(9):
        row = i//3
        col = i%3
        if isinstance(board.items[row][col], Dummy):
            if player == Computer:
                board.items[row][col] = X()
            else:
                board.items[row][col] = O()
            tester = minimax(-1*player, board)
            board.items[row][col] = Dummy()
...
class TicTacToe(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def computerTurn():
            ...
            possibleMoves = []
            #for i in range(9))
            maxMove = minimax(Computer,board)
            row, col = maxMove//3, maxMove%3
            board[row][col] = X(cv)
            self.locked = False
